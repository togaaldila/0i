...
+ Duplikasi code berikut
```python
from machine import Pin, PWM //Untuk membuat pin PWM, impor class PWM selain class Pin dari modul machine.
from time import sleep

frequency = 5000
led = PWM(Pin(5), frequency) //buat objek PWM yaitu led.

while True:
  for duty_cycle in range(0, 1024):
    led.duty(duty_cycle) // Untuk mengatur siklus kerja gunakan metode duty() pada objek PWM dan teruskan siklus tugas sebagai argumen:
    sleep(0.005)
```
